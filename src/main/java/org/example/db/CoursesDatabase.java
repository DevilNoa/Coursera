...
public class CoursesDatabase {
  ...
  public void createCourse(
      int id_courses, int id_instructor, String course_name, short total_time, short credit)
      throws SQLException {
    try {
      String sql =
          "INSERT INTO courses (id_courses, course_name, id_instructor, total_time, credits) VALUES (?, ?, ?, ?, ?)";
  ...
  public List<Courses> getAllCoursesAsList() {
    ...
    try {
      ...
      while (result.next()) {
        ...
        Courses course =
            new Courses(id_courses, course_name, id_instructor, time_created, total_time, credit);
  ...
  public Courses updateCourse(int id, Courses updatedCourse) throws SQLException {
    try {
      String sql =
          "UPDATE courses SET course_name = ?, id_instructor = ?, total_time = ?, credits = ? WHERE id_courses = ?";
      PreparedStatement statement = connection.prepareStatement(sql);
      ...
      if (affectedRows > 0) {
        ...
        return new Courses(
            ...
            updatedCourse.getId_instructor(),
            updatedCourse.getTime_created(),
            updatedCourse.getTotal_time(),
