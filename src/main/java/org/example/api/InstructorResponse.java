...
public class InstructorResponse {
  ...
  public Response addInstructor(
      @HeaderParam("Authorization") String token, Instructor newInstructor) {
    try {
      if (token == null || !token.startsWith("Bearer ")) {
        return Response.status(Response.Status.UNAUTHORIZED).build();
      }
  ...
  @Path("/{id}")
  @Consumes(MediaType.APPLICATION_JSON)
  public Response updateInstructors(
  ...
  public Response deleteInstructor(
      @HeaderParam("Authorization") String token, @PathParam("id") int id_instructors) {
    try {
      if (token == null || !token.startsWith("Bearer ")) {
        return Response.status(Response.Status.UNAUTHORIZED).build();
      }
      ...
      if (JwtService.verifyToken(jwtToken)) {
        ...
        if (success) {
          ...
          return Response.status(Response.Status.NOT_FOUND)
              .entity("Instructor not found or deletion failed")
              .build();
        }
      } else {
        return Response.status(Response.Status.UNAUTHORIZED).build();
